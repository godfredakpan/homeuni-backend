...
const Collections = () => {
  const [allCollections, setCollection] = useState([]);
  const [loading, setLoading] = useState(false);
  const [currentPage, setCurrentPage] = useState(0);
  const [collectionsPerPage] = useState(10); // Number of books to display per page
  const [searchTerm, setSearchTerm] = useState(""); // State to store the search term
  const [filteredCollections, setFilteredCollections] = useState([]); // State to store filtered books
  ...
  useEffect(() => {
    ...
    setFilteredCollections(sortedBooks);
  }, [searchTerm, sortBy, allCollections]);

  const indexOfLastCollection = (currentPage + 1) * collectionsPerPage;
  const indexOfFirstBook = indexOfLastCollection - collectionsPerPage;
  const currentCollections = filteredCollections.slice(indexOfFirstBook, indexOfLastCollection);
      ...
      <main className="min-h-screen flex-col items-center justify-between" style={{ margin: 20 }}>
        ...
        {sortBy === "category" && (
          ...
          >
            ...
            {Array.from(new Set(allCollections.map((book) => book.category))).map((category) => (
            ...
            <tbody>
              {currentCollections.map((collection) => (
                <tr className="bg-white border-b dark:bg-gray-800 dark:border-gray-700" key={collection.id}>
                  ...
                  <td className="px-6 py-4">{collection?.category}</td>
                  ...
                  <td className="px-6 py-4">
                    ...
                    <button
                      ...
                      onClick={() => deleteBookFunction(collection.id)}
                    >
                      Delete
                    </button>
                  </td>
        ...
        <ReactPaginate
          pageCount={Math.ceil(filteredCollections.length / collectionsPerPage)}
